...
- (id)initWithGameLayer:(GameLayer *)layer angle:(double)ag target:(CGPoint)tg {
    Tree *power = [[[layer gameData] treeMissile] objectAtIndex:0];
    ...
    switch ([power currentLevel]) {
        ...
        case 4:
        case 5:{
            if ((self = [super initWithSpriteFrameName:@"M_b_0001.png"])) {
            }
            break;
        }
        case 6:
    ...
    gameLayer = layer;
    [self setWeapon:[[layer gameData] missile]];
    target = tg;
...
- (void)removeSelf {
    ...
    // splash setting
    [crashZones removeAllChildrenWithCleanup:YES];
    CGFloat rangeOfSplash = GAME_METER*weapon.splashRange*0.5f;
    CrashZoneNode *zone = [[CrashZoneNode alloc] initAsCircleWithRadius:rangeOfSplash];
    [zone setPosition:CGPointMake(self.contentSize.width*0.5f,
                                  self.contentSize.height*0.5f)];
    [crashZones addChild:zone];
    [zone release];
    ...
    for (int i=0; i<[[[gameLayer enemyAirBatchNode] children] count]; i++) {
        if (isWillRemove) {
            ...
            if (isCrash && enemy.stateEnemy != StateEnemy_die) {
                [self hitEnemy:enemy damage:([self getDamageAir]*weapon.splashDamage)];
            }
        }
    }
    isCrash = NO;
    for (int i=0; i<[[[gameLayer enemyLandBatchNode] children] count]; i++) {
        if (isWillRemove) {
            ...
            isCrash = [GameUtils isCrashBullet:self character:enemy];
            if (isCrash && enemy.stateEnemy != StateEnemy_die) {
                [self hitEnemy:enemy damage:([self getDamageLand]*weapon.splashDamage)];
            }
        }
    }
...
- (void)moveSchedule:(ccTime)dt {
    ...
    if (self.position.x >= target.x) {
        distanceX -= 0.1f*GAME_METER*deltaTime;
        distanceY -= 9.8f*GAME_METER*deltaTime;
        if (distanceX <= 0) {
            distanceX = 0;
        }
    ...
    [self setPosition:CGPointMake(self.position.x
                                  ...
                                  + (distanceY*deltaTime))];
    [crashZones setPosition:self.position];
    CGPoint missileSmoke = CGPointMake(10*cosf(angle), 10*sinf(angle));
    [gameLayer addMissileSmoke:CGPointMake(self.position.x - missileSmoke.x,
                                           self.position.y - missileSmoke.y)];
}
